	...
	float3* __restrict__ dL_conic_another)
{
	...
	__shared__ float3 collected_normal[3 * BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float power = -0.5f * (con_o1.x * d.x * d.x + con_o1.z * d.y * d.y) - con_o1.y * d.x * d.y;
			if (power > 0.0f)
				continue;

            float denomi_z = norm_use.z;

            const float temp = ((norm_use.x+con_o2.x)*d.x + (norm_use.y+con_o2.y)*d.y);
			const float tanh_result1 = erff(temp);
            const float tanh_result2 = -tanh_result1;
            const float dev_erf1 = 2/sqrt(3.141592654f) * exp(-temp*temp);
            const float dev_erf2 = dev_erf1;
            const float exp_part = exp(power);

            const float G1 = exp_part*(1+tanh_result1);
            const float G2 = exp_part*(1+tanh_result2);

			const float alpha1 = con_o1.w * G1;
			const float alpha2 = con_o2.w * G2;
			const float alpha = min(0.99f,0.5f*(alpha1 + alpha2));

            const float dG1_dx = (0.5f*W)*(-(con_o1.x*d.x + con_o1.y*d.y)*G1 + exp_part*dev_erf1*(norm_use.x+con_o2.x));
            const float dG2_dx = (0.5f*W)*(-(con_o1.x*d.x + con_o1.y*d.y)*G2 - exp_part*dev_erf2*(norm_use.x+con_o2.x));
            const float dG1_dy = (0.5f*H)*(-(con_o1.z*d.y + con_o1.y*d.x)*G1 + exp_part*dev_erf1*(norm_use.y+con_o2.y));
            const float dG2_dy = (0.5f*H)*(-(con_o1.z*d.y + con_o1.y*d.x)*G2 - exp_part*dev_erf1*(norm_use.y+con_o2.y));

            const float dG1_dcx = -(0.5f*d.x*d.x)*G1 + exp_part*dev_erf1*(0.5f*sigma6*sigma6*1.99999982358225f*con_o2.z*con_o2.z*temp + con_o2.z*sigma6*d.x);
            const float dG2_dcx = -(0.5f*d.x*d.x)*G2 - exp_part*dev_erf2*(0.5f*sigma6*sigma6*1.99999982358225f*con_o2.z*con_o2.z*temp + con_o2.z*sigma6*d.x);
            const float dG1_dcy = -(0.5f*d.x*d.y)*G1 + 0.5f*exp_part*dev_erf1*(sigma6*sigma7*1.99999982358225f*con_o2.z*con_o2.z*temp + con_o2.z*(sigma7*d.x+sigma6*d.y));
            const float dG2_dcy = -(0.5f*d.x*d.y)*G2 - 0.5f*exp_part*dev_erf2*(sigma6*sigma7*1.99999982358225f*con_o2.z*con_o2.z*temp + con_o2.z*(sigma7*d.x+sigma6*d.y));
            const float dG1_dcz = -(0.5f*d.y*d.y)*G1 + exp_part*dev_erf1*(0.5f*sigma7*sigma7*1.99999982358225f*con_o2.z*con_o2.z*temp + con_o2.z*sigma7*d.y);
            const float dG2_dcz = -(0.5f*d.y*d.y)*G2 - exp_part*dev_erf2*(0.5f*sigma7*sigma7*1.99999982358225f*con_o2.z*con_o2.z*temp + con_o2.z*sigma7*d.y);

            const float dG1_dn1 =  exp_part*dev_erf1*d.x*denomi_z;
            const float dG1_dn2 =  exp_part*dev_erf1*d.y*denomi_z;
            const float dG1_dn3 =  -exp_part*dev_erf1*((norm_use.x*d.x + norm_use.y*d.y)*norm_use.z/(con_o2.z));

            const float dG2_dn1 =  -dG1_dn1;
            const float dG2_dn2 =  -dG1_dn2;
            const float dG2_dn3 =  -dG1_dn3;
			...
			atomicAdd(&dL_dmean2D[global_id].x, dL_dalpha*(0.5f*con_o1.w*dG1_dx + 0.5f*con_o2.w*dG2_dx));
			atomicAdd(&dL_dmean2D[global_id].y, dL_dalpha*(0.5f*con_o1.w*dG1_dy + 0.5f*con_o2.w*dG2_dy));

			atomicAdd(&dL_dconic2D[global_id].x, dL_dalpha*(0.5f*con_o1.w*dG1_dcx + 0.5f*con_o2.w*dG2_dcx));
			atomicAdd(&dL_dconic2D[global_id].y, dL_dalpha*(0.5f*con_o1.w*dG1_dcy + 0.5f*con_o2.w*dG2_dcy));
			atomicAdd(&dL_dconic2D[global_id].w, dL_dalpha*(0.5f*con_o1.w*dG1_dcz + 0.5f*con_o2.w*dG2_dcz));

			atomicAdd(&(dL_dopacity1[global_id]), 0.5f * G1 * dL_dalpha);
			atomicAdd(&(dL_dopacity2[global_id]), 0.5f * G2 * dL_dalpha);

            atomicAdd(&dL_dnormal[global_id].x, 0.5f* dL_dalpha*(con_o1.w*dG1_dn1 + con_o2.w*dG2_dn1));
            atomicAdd(&dL_dnormal[global_id].y, 0.5f* dL_dalpha*(con_o1.w*dG1_dn2 + con_o2.w*dG2_dn2));
            atomicAdd(&dL_dnormal[global_id].z, 0.5f* dL_dalpha*(con_o1.w*dG1_dn3 + con_o2.w*dG2_dn3));

            atomicAdd(&dL_conic_another[global_id].x, 0.5f* dL_dalpha*(con_o1.w*dG1_ds6 + con_o2.w*dG2_ds6));
            atomicAdd(&dL_conic_another[global_id].y, 0.5f* dL_dalpha*(con_o1.w*dG1_ds7 + con_o2.w*dG2_ds7));
            atomicAdd(&dL_conic_another[global_id].z, 0.5f* dL_dalpha*(con_o1.w*dG1_ds8 + con_o2.w*dG2_ds8));
