...
__device__ void generate_type(float3 cov, float3 cov_small, float op1, float op2, float& type, float3 normal, float x_term, float y_term, float3 conic)
{
    ...
    float full = (fabsf(op1-op2)<0.0039)?0.f:1.f;
    ...
    float point3_large= 1.f - point1_large;
    float point4_large= 1.f - point2_large;
...
__device__ void calculate_small(float6 cov_input, const float* viewmatrix, float3 normal, float* cov_new_small) {
    ...
    glm::mat3 cov;
    cov[0][0] = cov_input.x;
    cov[0][1] = cov[1][0] = cov_input.y;
    cov[0][2] = cov[2][0] = cov_input.w;
    cov[1][1] = cov_input.z;
    cov[1][2] = cov[2][1] = cov_input.u;
    cov[2][2] = cov_input.v;
	...
	float3* save_normal)
{
	...
	float3 gs_normal = {normal[3*idx],normal[3*idx+1],normal[3*idx+2]};
	...
	float3 conic = { cov.z * det_inv2, -cov.y * det_inv2, cov.x * det_inv2 };
    float power = logf(256.f * max(opacities[2 * idx], opacities[2 * idx+1]));
    ...
    if (cov3D_precomp_small != nullptr && (2*width > BLOCK_X || 2*height > BLOCK_Y))
	{
        calculate_small(cov_temp, viewmatrix, gs_normal, cov3D_smalls + idx * 6);
        cov3D_small = cov3D_smalls + idx * 6;
        cov_small = {cov3D_small[0],cov3D_small[1],cov3D_small[3]};
    }
    else
	{
        cov_small = cov;
    }
	float lam1 = cov_small.x;
	float lam2 = cov_small.z;
    ...
    height_small = 2*height_small>height ? min(height_small, height):max(height_small, height);
    if (width <= 0 || height <= 0){
        return;
    }

    float power2 = logf(256.f * min(opacities[2 * idx], opacities[2 * idx+1]));
    ...
    height_small_another = 2*height_small_another>height_another?min(height_small_another, height_another):max(height_small_another, height_another);
	...
	gs_normal.z = 1.f/(1.4142135f*(gs_normal.z+0.000001f));
	...
	const float3* __restrict__ normal)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float4 con_o1 = collected_conic_opacity1[j];
			...
			float alpha = min(0.99f,0.5f*(alpha1 + alpha2));
